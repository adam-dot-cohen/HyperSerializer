namespace HyperSerializer.CodeGen.Snipets
{
    internal class SnippetsUnsafe : ISnippetsSafeV3
    {
        ...
        public string PropertyTemplateSerializeNullable { get { return "var _{0} = {1} ?? default; offset+=offsetWritten; if(((bytes[offset++] = (byte)({1}==null ? 1 : 0)) != 1)) fixed (byte* p = bytes.Slice(offset+=offsetWritten, offsetWritten = {2})) *(({3}?*)(p)) = _{0}; else offsetWritten = 0;"; } }
        ...
        public string StringLengthSpan { get { return "({0}?.Length * 2 ?? 0)"; } }
